...
- **Global scope**: `C:/Users/Eric/dev/mini-code/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Users/Eric/dev/mini-code/.claude/notes/`)
...
2. Glob `C:/Users/Eric/dev/mini-code/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Users/Eric/dev/mini-code/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Users/Eric/dev/mini-code/.claude/notes/` — works in any directory
